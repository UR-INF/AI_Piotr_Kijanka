<?php
	...
	if(ISSET($_POST['add_guest'])){
		...
		$query2 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$checkin' && `room_id` = '$_REQUEST[room_id]' && `status` = 'W oczekiwaniu'") or die(mysqli_error());
		$row = $query2->num_rows;
		if($checkin < date("Y-m-d", strtotime('+8 HOURS'))){	
				echo "<script>alert('Data musi być aktualna')</script>";
			}else{
				if($row > 0){
					echo "<div class = 'col-md-4'>
								<label style = 'color:#ff0000;'>Data jest niedostępna</label><br />";
							$q_date = $conn->query("SELECT * FROM `transaction` WHERE `status` = 'W oczekiwaniu'") or die(mysqli_error());
						...
						if($guest_id = $fetch['guest_id']){
							...
							$conn->query("INSERT INTO `transaction`(guest_id, room_id, status, checkin) VALUES('$guest_id', '$room_id', 'W oczekiwaniu', '$checkin')") or die(mysqli_error());
